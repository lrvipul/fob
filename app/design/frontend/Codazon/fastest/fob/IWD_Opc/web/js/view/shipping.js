define(
    ...
    function ($,
        ...
        return Component.extend({
                ...
                this.rates.subscribe(function (rates) {
                    ...
                    _.each(rates, function (rate) {
                        if (rate) {
                            ...
                            var visible = true;
                            if(rate.carrier_code == "pickupatstore"){
                                visible = false;
                            }
                            if (self.shippingRateGroups.indexOf(carrierTitle) === -1) {
                                self.shippingRateGroups.push({label: carrierTitle, code: rate.method_code, visible: visible});
                            }
                        }
                    });
            ...
            optionsRenderAfter: function(option, item) {
                        ...
                        if (PickupAtStore.isPASSelected()) {
                            if (!customer.isLoggedIn()) {
                                ...
                                /*if(typeof pos !== "undefined"){
                                    $('[name="shippingAddress.firstname"] input').val(pos.firstname).change();
                                    $('[name="shippingAddress.lastname"] input').val(pos.lastname).change();
                                    $('[name="shippingAddress.company"] input').val(pos.company).change();
                                    $('[name="shippingAddress.street.0"] input').val(pos.street_1).change();
                                    $('[name="shippingAddress.street.1"] input').val(pos.street_2).change();
                                    $('[name="shippingAddress.city"] input').val(pos.city).change();
                                    $('[name="shippingAddress.region_id"] select').val(pos.regionId).change();
                                    $('[name="shippingAddress.region"] input').val(pos.region).change();
                                    $('[name="shippingAddress.postcode"] input').val(pos.postcode).change();
                                    $('[name="shippingAddress.country_id"] select').val(pos.countryId).change();
                                    $('[name="shippingAddress.telephone"] input').val(pos.telephone).change();
                                }*/
                            }                                                                                
                        }
            ...
            validateShippingInformation: function (showErrors) {
                ...
                if (typeof (PickupAtStore) !== "undefined") {
                    if (PickupAtStore.isPASSelected()) {
                        $('#pas-pos-selector').closest("#pas-pos").find('.field-error').remove();
                        var error = false;
                        if (typeof PickupAtStore.method.store == "undefined" || $('#pas-pos-selector').val() == 0   || PickupAtStore.method.store == "0") {
                            if (PickupAtStore.config.dropdown == "1") {
                                ...
                                $('#pas-pos-selector').closest("#pas-pos").find('.field-error').remove();
                                $('#pas-pos-selector').closest("#pas-pos").append('<div class="field-error"><span>Please select store.</span></div>');
                            } else {
                                $('#pas-pos-selector').closest("#pas-pos").find('.field-error').remove();
                        ...
                        if (PickupAtStore.config.dropdown == "1") {
                            if (PickupAtStore.config.date == "1" && (typeof PickupAtStore.method.date == "undefined" || PickupAtStore.method.date == "0")) {
                                ...
                                $('#pas-date-selector').closest("#pas-date").find('.field-error').remove();
                                $('#pas-date-selector').closest("#pas-date").append('<div class="field-error"><span>Please select date.</span></div>');
                                error = true;
                            }else{
                                $('#pas-date-selector').closest("#pas-date").find('.field-error').remove();
                            }
                            if (PickupAtStore.config.date == "1" && (PickupAtStore.config.time == "1" && (typeof PickupAtStore.method.time == "undefined" || PickupAtStore.method.time == "0"))) {
                                ...
                                $('#pas-time-selector').closest("#pas-time").find('.field-error').remove();
                                $('#pas-time-selector').closest("#pas-time").append('<div class="field-error"><span>Please select time.</span></div>');
                                error = true;
                            }else{
                                $('#pas-time-selector').closest("#pas-time").find('.field-error').remove();
                            }

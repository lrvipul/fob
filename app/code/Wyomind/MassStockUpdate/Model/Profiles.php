...
class Profiles extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function import($nth, $filePath)
    {
        try {
            ...
            if ($profileMethod == 0) {
                $profileMethod = $helperClass::UPDATE;
            }
            $this->_products = [];
            ...
            $productCollection = $this->_productCollectionFactory->create()->getSkuAndIdentifierCollection($this->_identifierCode);
            ...
            foreach ($helperClass::MODULES as $module) {
            ...
            foreach ($modules as $module) {
                ...
                if ($this->getpostProcessIndexers() > $helperClass::POST_PROCESS_INDEXERS_DISABLED) {
                    if ($this->getpostProcessIndexers() == $helperClass::POST_PROCESS_INDEXERS_AUTOMATICALLY) {
                        $indexes = array_unique($indexes + $resource->getIndexes($columns));
                    } elseif ($this->getpostProcessIndexers() == $helperClass::POST_PROCESS_INDEXERS_ONLY_SELECTED) {
            ...
            $moduleByOrder = $helperClass::MODULES;
            ksort($moduleByOrder);
...
");
    ...
    public function createSql($cell, $modules, $columns, $profileMethod)
    {
        ...
        try {
            ...
            $updateOnly = $profileMethod == $helperClass::UPDATE;
            $importOnly = $profileMethod == $helperClass::IMPORT;
            $updateAndImport = $profileMethod == $helperClass::UPDATEIMPORT;
            if ($updateOnly && $productExists || $importOnly && !$productExists || $updateAndImport) {
                ...
                if ($productExists) {
                    ...
                    $resource->collect($productId, $identifier, array("option" => array($this->_identifierCode)), $this);
                }
                if ($parentExists) {
                    if (isset($parentSku)) {
                    ...
                    if (isset($parentSku)) {
                        if ($this->getCreateConfigurableOnthefly()) {
                            $configurableProductResource->collect($productId, $parentSku, array("option" => array($this->_identifierCode)), $this);
                        }
                    }
                }
                // the product exist !
                ...
                foreach ($columns as $column) {
                    ...
                    if ($column->enabled) {
                         ...
                         */
                        if (in_array($column->configurable, array(0, 2))) {
                            if (!$productExists) {
                                $sku = $identifier;
                        ...
                        if ($this->getCreateConfigurableOnthefly()) {
                            ...
                            if (in_array($column->configurable, array(1, 2))) {
                                ...
                                if (isset($parentSku) && !in_array($parentSku, $this->parentSku)) {
                                    if (!$parentExists) {
                                        $sku = $parentSku;
                                        if ($this->_identifierCode == "sku") {
                                            $id = "(SELECT entity_id FROM " . $this->tableCpe . " WHERE sku=" . $this->_dataHelper->sanitizeField($sku) . " LIMIT 1)";
                ...
                foreach ($modules as $module) {
                     ...
                     */
                    if (stristr($module, "ConfigurableProducts") != FALSE && (!$this->getCreateConfigurableOnthefly() || !isset($parentSku) || in_array($parentSku, $this->parentSku))) {
                        continue;
                    }
                ...
                if ($productExists) {
                    $this->_notices[] = $identifier;
                } else {
                    $this->_success[] = $identifier;
                }
            } else {
    ...
    public function postProcess()
    {
        ...
        if ($this->_params['post_process_action'] == $helperClass::POST_PROCESS_ACTION_MOVE) {
            if ($this->_params['file_system_type'] == $helperClass::LOCATION_MAGENTO) {
            ...
            } elseif ($this->_params['file_system_type'] == $helperClass::LOCATION_FTP) {
                $this->_storageHelper->ftpFileAction($this->_params, 'move');
            }
        } elseif ($this->_params['post_process_action'] == $helperClass::POST_PROCESS_ACTION_DELETE) {
            if ($this->_params['file_system_type'] == $helperClass::LOCATION_MAGENTO) {

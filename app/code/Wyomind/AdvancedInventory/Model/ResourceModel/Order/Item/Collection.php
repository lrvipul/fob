...
class Collection extends \Magento\Sales\Model\ResourceModel\Order\Item\Collection
{
    ...
    public function getAssignationByOrderId($orderId, $itemId = false)
    {
        ...
        $this->getSelect()->columns(["name" => "name", "sku" => "sku", "order_id" => "order_id", "item_id" => "item_id", "product_id" => "product_id", "product_type" => "product_type", "qty_ordered" => "qty_ordered", "qty_canceled" => new \Zend_Db_Expr("IF(ISNULL(parent_item_id),`main_table`.`qty_canceled`,(SELECT soi.qty_canceled FROM {$salesOrderItem} AS soi WHERE soi.item_id=main_table.parent_item_id))"), "qty_refunded" => new \Zend_Db_Expr("IF(ISNULL(parent_item_id),`main_table`.`qty_refunded`,(SELECT soi.qty_refunded FROM {$salesOrderItem} AS soi WHERE soi.item_id=main_table.parent_item_id))"), "pre_assignation" => new \Zend_Db_Expr("IF(ISNULL(parent_item_id),`main_table`.`pre_assignation`,(SELECT soi.pre_assignation from {$salesOrderItem} AS soi WHERE soi.item_id=main_table.parent_item_id))")])->joinLeft(['advancedinventory_assignation' => $advancedinventoryAssignation], 'main_table.item_id=advancedinventory_assignation.item_id', ["qty_unassigned" => new \Zend_Db_Expr("(qty_ordered - IF(ISNULL(parent_item_id),`main_table`.`qty_canceled`,(SELECT soi.qty_canceled FROM {$salesOrderItem} AS soi WHERE soi.item_id=main_table.parent_item_id)) - IF(ISNULL(parent_item_id),`main_table`.`qty_refunded`,(SELECT soi.qty_refunded FROM {$salesOrderItem} AS soi WHERE soi.item_id=main_table.parent_item_id))) - (SUM(IFNULL(qty_assigned,0)) - SUM(IFNULL(advancedinventory_assignation.qty_returned,0)))"), "qty_to_assign" => new \Zend_Db_Expr("(qty_ordered - IF(ISNULL(parent_item_id),`main_table`.`qty_canceled`,(SELECT soi.qty_canceled FROM {$salesOrderItem} AS soi WHERE soi.item_id=main_table.parent_item_id)) - IF(ISNULL(parent_item_id),`main_table`.`qty_refunded`,(SELECT soi.qty_refunded FROM {$salesOrderItem} AS soi WHERE soi.item_id=main_table.parent_item_id)))"), "qty_assigned" => new \Zend_Db_Expr("SUM(IFNULL(qty_assigned,0))"), "qty_returned" => new \Zend_Db_Expr("SUM(IFNULL(advancedinventory_assignation.qty_returned,0))"), "place_id" => "place_id"], null, 'left')->joinLeft(['advancedinventory_item' => $advancedinventoryItem], 'main_table.product_id=advancedinventory_item.product_id', ["multistock_enabled" => "multistock_enabled"], null, 'left')->group("main_table.item_id");
        return $this;
    }
}
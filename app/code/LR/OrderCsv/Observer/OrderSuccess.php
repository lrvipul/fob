...
class OrderSuccess implements \Magento\Framework\Event\ObserverInterface
{
   ...
   {
      $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/OrderCSV.log');
      ...
      //$logger->info(print_r($orderData->getData(), true));
      $logger->info("Order ID ==> ".$orderId);
      $order = $this->_orderFactory->load($orderId); 
	  $logger->info("Order ==> ".$order->getShippingMethod());
      ...
      {*/
        /* if($order->getPickupStore() != '')
         {*/
            ...
            $logger->info("Order IncrementID => ".$orderIncrementId);
